...
function SparklineChart({ ticker, color, timeframe }: { ticker: string, color: string, timeframe: '1d' | '1w' | '1m' }) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/historical?ticker=${ticker}&timeframe=${timeframe}`);
        if (res.ok) {
...
export default function LandingView({ setActiveTab, onSelectAsset, onOpenAuthModal }: LandingViewProps) {
  ...
  // Simulated Sandbox Terminal Persistent state
  const [simSandboxBalance, setSimSandboxBalance] = useState(100000);
  const [simSandboxTicker, setSimSandboxTicker] = useState('NVDA');
  const [simSandboxTrades, setSimSandboxTrades] = useState<any[]>([]);
  ...
  const fetchLiveMarketPulse = async () => {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        setPulseLiveNews(data.headlines || []);
        setIsPulseGrounded(!!data.grounded);
      }
    } catch (e) {
  ...
  return (
    <div className="space-y-12 pb-12">
      ...
      <div className="relative rounded-3xl overflow-hidden border border-zinc-800 bg-[#0C0C0E] bg-radial from-[#1e1003] via-[#0C0C0E] to-[#0C0C0E] px-6 py-16 sm:px-12 sm:py-24 text-center">
        {/* Absolute ambient glow points */}
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[550px] h-[300px] bg-gradient-to-b from-[#FE8C00]/10 to-transparent blur-3xl pointer-events-none" />
        <div className="absolute -bottom-10 left-12 w-[220px] h-[220px] bg-amber-500/5 blur-3xl pointer-events-none" />
        ...
        <motion.div 
          ...
          className="inline-flex items-center gap-2 rounded-full border border-zinc-800 bg-zinc-900/60 px-4 py-1.5 text-xs font-semibold text-zinc-305"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="space-y-3.5">
              ...
              <div className="rounded-xl border border-zinc-900 bg-zinc-950/60 p-4 space-y-2.5 relative">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[8px] font-mono text-zinc-650">May 2026</span>
                </div>
        ...
        <div className="lg:col-span-12 xl:col-span-7 rounded-2xl border border-zinc-900 bg-[#0C0C0E] bg-radial from-[#0c1611] to-[#0C0C0E] p-6 text-left relative overflow-hidden flex flex-col justify-between space-y-6">
          ...
          <div className="space-y-4">
            ...
            <h3 className="font-display text-lg sm:text-2xl font-black text-white leading-tight">
            ...
            {/* Structured Proof-of-Work list */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 pt-1.5">
              <div className="p-3.5 border border-zinc-900/60 bg-zinc-950/30 rounded-xl space-y-1 hover:border-emerald-500/20 transition-colors">
                <span className="text-[9px] font-mono text-emerald-400 uppercase font-black tracking-wider block">1. banking forex swap buffers</span>
                <p className="text-[10.5px] text-zinc-300 font-semibold leading-normal">
                  Scratched banking cashbooks for Tier-1 banks (GTCO/Zenith bank) to measure currency hedging buffers.
                </p>
              </div>

              <div className="p-3.5 border border-zinc-900/60 bg-zinc-950/30 rounded-xl space-y-1 hover:border-amber-500/20 transition-colors">
                <span className="text-[9px] font-mono text-amber-400 uppercase font-black tracking-wider block">2. macro indicator surveys</span>
                <p className="text-[10.5px] text-zinc-300 font-semibold leading-normal">
                  Sourcing item values across primary retail nodes in Nigeria to pre-calculate interest-rate pivots.
                </p>
              </div>

              <div className="p-3.5 border border-zinc-900/60 bg-zinc-950/30 rounded-xl space-y-1 hover:border-sky-500/20 transition-colors">
                <span className="text-[9px] font-mono text-sky-400 uppercase font-black tracking-wider block">3. liquidity desk tracking</span>
                <p className="text-[10.5px] text-zinc-300 font-semibold leading-normal">
                  Tracking OTC crypto reserves and spot market vacuum nodes to isolate high-conviction holds.
                </p>
              </div>

              <div className="p-3.5 border border-zinc-900/60 bg-zinc-950/30 rounded-xl space-y-1 hover:border-indigo-500/20 transition-colors">
                <span className="text-[9px] font-mono text-indigo-400 uppercase font-black tracking-wider block">4. double dual-listed pegs</span>
                <p className="text-[10.5px] text-zinc-300 font-semibold leading-normal">
                  Analyzing raw cash multipliers for export miners yielding foreign currency dividends.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-zinc-900 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="text-[10px] text-zinc-500 font-mono tracking-wide uppercase font-bold">
              👥 Join 30,000+ disciplined strategists doing fundamental research
            </div>
            <button
      ...
      <div className="rounded-2xl border border-zinc-900 bg-zinc-950/20 p-5 sm:p-6 text-left space-y-4 relative overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <button
            ...
            className="self-start sm:self-center flex items-center gap-2 rounded-lg border border-zinc-800 bg-zinc-905/40 hover:bg-zinc-950 text-neutral-300 hover:text-white px-3 py-1.5 text-xs font-mono font-bold transition-all disabled:opacity-50 cursor-pointer"
          >
            ...
            <div className="ticker-track flex gap-12 shrink-0">
              ...
              {[...pulseLiveNews, ...pulseLiveNews].map((news, i) => (
                <span key={i} className="flex items-center gap-2 font-bold uppercase tracking-wide">
                  ...
                  <span className="text-neutral-400 text-[9px] bg-zinc-90 w-fit px-1.5 py-0.5 rounded border border-zinc-800">{news.source}</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[1, 2, 3, 4].map((n) => (
              <div key={n} className="rounded-xl border border-zinc-900/60 bg-zinc-900/10 p-4 space-y-3 animate-pulse">
                ...
                <div className="h-10 w-full bg-zinc-905/40 rounded border border-zinc-800/40" />
              </div>
            ))}
          </div>
        ) : (
              ...
              >
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-[10px] font-mono">
                    <span className="rounded bg-zinc-90 px-2 py-0.5 border border-zinc-800 text-neutral-300 font-bold group-hover/pulse-card:border-[#FE8C00]/30 group-hover/pulse-card:text-[#FE8C00] transition-colors">
                      {news.source}
                    </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {coreFeatures.map((feat, index) => (
            <motion.div
              ...
              className="group relative cursor-pointer rounded-2xl border border-zinc-800/80 bg-zinc-950/80 p-6 flex flex-col justify-between text-left h-64 overflow-hidden"
            >
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="p-2.5 rounded-xl border border-zinc-80) bg-zinc-900/60 group-hover:scale-110 transition-transform duration-300">
                    {feat.icon}
                  </div>
      ...
      <div id="sgt-roadmaps-section" className="rounded-2xl border border-zinc-800 bg-[#0C0C0E] bg-radial from-[#120e09] to-[#0C0C0E] p-6 lg:p-8 text-left space-y-8 relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-zinc-900 pb-5">
          ...
          <div className="flex flex-wrap gap-1 bg-zinc-950 p-1 rounded-xl border border-zinc-900 w-fit">
            {(['all', 'signals', 'community', 'infrastructure'] as const).map((filter) => (
              <button
                key={filter}
                onClick={() => {
              ...
              >
                {filter === 'all' ? 'All Pipelines' : filter === 'signals' ? 'Signals & AI' : filter === 'community' ? 'Reputation' : 'OTC Ledger'}
              </button>
            ))}
          </div>
        </div>
        ...
        <AnimatePresence>
          {activeRoadmapSimulationId && (
            ...
            >
              ...
              {/* SIMULATOR C: P2P Secondary Ledger Ledger Block Swaps */}
              {activeRoadmapSimulationId === 'rm-p2p-ledger' && (
                <div className="space-y-4">
                  <div>
                    <h4 className="text-xs font-bold text-sky-400 uppercase font-mono tracking-wider">
                      🏦 P2P Escrow Block Matcher [PROTOPREVIEW]
                    </h4>
                    <p className="text-[11px] text-zinc-400 mt-1">
                      Simulate posting a secondary private trade order block of listed banking shares. Watch are matching engine match or lock execution automatically within seconds.
                    </p>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-12 gap-5 align-top">
                    {/* Input Area */}
                    <div className="md:col-span-5 bg-zinc-950 border border-zinc-900 rounded-xl p-4 space-y-3">
                      <div className="space-y-1">
                        <label className="text-[9.5px] font-mono text-zinc-500 uppercase font-black tracking-wide block">Select Core Stock Block</label>
                        <select className="w-full bg-zinc-900 border border-zinc-800 text-xs text-white p-2 rounded-lg focus:outline-none">
                          <option value="ZENITHBANK">Zenith Bank Plc (ZENITHBANK)</option>
                          <option value="GTCO">GTCO Holdings Plc (GTCO)</option>
                          <option value="DANGCEM">Dangote Cement Plc (DANGCEM)</option>
                        </select>
                      </div>

                      <div className="grid grid-cols-2 gap-2.5">
                        <div className="space-y-1">
                          <label className="text-[9px] font-mono text-zinc-500 uppercase font-black block">Quantity (Units)</label>
                          <input 
                            type="number" 
                            value={simOfferQty} 
                            onChange={(e) => setSimOfferQty(Number(e.target.value))}
                            className="w-full bg-zinc-900 border border-zinc-800 text-xs text-white p-2 rounded-lg"
                          />
                        </div>
                        <div className="space-y-1">
                          <label className="text-[9px] font-mono text-zinc-505 uppercase font-black block">Price (NGN / unit)</label>
                          <input 
                            type="number" 
                            step="0.05"
                            value={simOfferPrice} 
                            onChange={(e) => setSimOfferPrice(Number(e.target.value))}
                            className="w-full bg-zinc-900 border border-zinc-800 text-xs text-white p-2 rounded-lg"
                          />
                        </div>
                      </div>
                      ...
                      <div className="pt-2">
                        <button
                          onClick={() => {
                            setSimOfferSubmitted(true);
                            const tEvent = new CustomEvent('show-toast', {
                              detail: {
                                message: `Broadcasted block of ${simOfferQty.toLocaleString()} units at ${simOfferPrice.toFixed(2)} NGN to secondary escrow boards!`,
                                type: 'success'
                              }
                            });
                            ...
                            setTimeout(() => {
                              const matchEvent = new CustomEvent('show-toast', {
                                detail: {
                                  message: `MATCHED SUCCESS: SGT operator "Sout_Lag_Alpha" matched and accepted your block escrow transaction!`,
                                  type: 'success'
                                }
                              });
                          ...
                          disabled={simOfferSubmitted}
                          className="w-full text-xs font-mono font-black text-zinc-950 bg-sky-400 hover:bg-sky-500 py-3 rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer flex items-center justify-center gap-1.5"
                        >
                          <Shield className="h-3.5 w-3.5" />
                          {simOfferSubmitted ? "Locked & Broadcasting Block" : "Submit Peer Transaction Block"}
                        </button>
                      </div>
                    </div>

                    {/* Live matching feedback console */}
                    <div className="md:col-span-7 rounded-xl bg-zinc-950 border border-zinc-900 p-4 flex flex-col justify-between min-h-[175px]">
                      <div className="space-y-2.5">
                        <span className="text-[9px] font-mono text-sky-400 tracking-wider font-extrabold uppercase block">
                          📡 OTC LIQUIDITY BROADCAST LEDGER
                        </span>

                        <div className="space-y-2">
                          <div className="flex justify-between items-center text-[10.5px] font-semibold border-b border-zinc-900 pb-1.5">
                            <span className="text-zinc-500">PEER NETWORK:</span>
                            <span className="text-emerald-400 font-mono font-black animate-pulse">● SWAP BOARD CONNECTED</span>
                          </div>

                          <div className="space-y-1 text-[10px] font-mono leading-relaxed">
                            {simOfferSubmitted ? (
                              <>
                                <p className="text-emerald-400">▸ SUCCESS: Order recorded securely inside ledger chain.</p>
                                <p className="text-sky-400 animate-pulse">▸ BROADCASTING: Matching ${simOfferQty.toLocaleString()} units on secondary market liquidity boards...</p>
                                <p className="text-zinc-500 mt-1 select-none">⏳ Waiting approx 5s for peer signatures...</p>
                              </>
                            ) : (
                              <p className="text-zinc-500 italic">No block offers active. Configure shares on the left to broadcast to global SGT scouts.</p>
                            )}
                          </div>
                        </div>
                      </div>
                      ...
                      {simOfferSubmitted && (
                        <div className="p-2 border border-sky-505/20 bg-sky-500/5 rounded-lg text-[9.5px] text-zinc-400 leading-normal font-semibold mt-2 flex items-center justify-between">
                          <span>Verified Escrow Reference: <span className="text-sky-300 font-mono">#ESC-NGX-{Math.floor(Math.random()*90000+10000)}</span></span>
                          <button 
                            onClick={() => setSimOfferSubmitted(false)}
                            className="text-[8.5px] uppercase font-mono font-black text-sky-400 hover:underline cursor-pointer"
                          >
                            Reset Form
                          </button>
                        </div>
              ...
              {/* SIMULATOR D: Simulated Broker API Sandbox */}
              {activeRoadmapSimulationId === 'rm-sandbox-terminal' && (
                <div className="space-y-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1">
                    <div>
                      <h4 className="text-xs font-bold text-indigo-400 uppercase font-mono tracking-wider">
                        💻 simulated multi-broker trading sandbox
                      </h4>
                      <p className="text-[11.5px] text-zinc-400 mt-1">
                        Test executing instant equity purchases. Experience exact pricing adjustments and latency slippage before trade validation.
                      </p>
                    </div>
                    <div className="bg-zinc-900 border border-zinc-800 p-2 rounded-xl text-center shrink-0">
                      <span className="text-[8px] font-mono text-zinc-500 uppercase block font-bold">Simulated Balance</span>
                      <span className="text-sm font-mono font-extrabold text-white">${simSandboxBalance.toLocaleString()}</span>
                    </div>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-12 gap-5 align-top">
                    {/* Buy/Sell Controls */}
                    <div className="md:col-span-5 bg-zinc-950 border border-zinc-900 p-4 rounded-xl space-y-3 text-xs">
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          <label className="text-[9.5px] font-mono text-zinc-500 uppercase font-black block mb-1">Stock Ticker</label>
                          <select 
                            value={simSandboxTicker}
                            onChange={(e) => setSimSandboxTicker(e.target.value)}
                            className="w-full bg-zinc-900 border border-zinc-800 p-2 rounded-lg text-white font-mono focus:outline-none focus:border-indigo-500 cursor-pointer"
                          >
                            <option value="NVDA">NVDA ($940.25)</option>
                            <option value="AAPL">AAPL ($178.50)</option>
                            <option value="BTC">BTC ($66,530)</option>
                          </select>
                        </div>
                        <div>
                          <label className="text-[9.5px] font-mono text-zinc-500 uppercase font-black block mb-1">Action Type</label>
                          <span className="w-full flex items-center justify-center p-2 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 font-mono font-black select-none">
                            BUY ACQUIRE
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="pt-2">
                        <button
                          onClick={() => {
                            const stepPrice = simSandboxTicker === 'NVDA' ? 940.25 : simSandboxTicker === 'AAPL' ? 178.50 : 66530;
                            const isCrypto = simSandboxTicker === 'BTC';
                            const simQuantity = isCrypto ? 0.05 : 10;
                            const totalVal = stepPrice * simQuantity;

                            if (simSandboxBalance < totalVal) {
                              const errEvt = new CustomEvent('show-toast', {
                                detail: {
                                  message: "Error: Insufficient simulated funds!",
                                  type: 'error'
                                }
                              });
                            ...
                            // Update Trades & Balance
                            setSimSandboxBalance(prev => prev - totalVal);
                            const newTrade = {
                              id: `TX-${Date.now().toString().slice(-5)}`,
                              ticker: simSandboxTicker,
                              qty: simQuantity,
                              price: stepPrice,
                              slippage: "+0.11% USD delay",
                              total: totalVal,
                              time: new Date().toLocaleTimeString()
                            };
                            setSimSandboxTrades(prev => [newTrade, ...prev]);

                            const successEvt = new CustomEvent('show-toast', {
                              detail: {
                                message: `Simulated sandbox execution success: filled ${simQuantity} units of ${simSandboxTicker}!`,
                                type: 'success'
                              }
                            });
                            window.dispatchEvent(successEvt);
                          }}
                          className="w-full text-xs font-mono font-black text-zinc-950 bg-indigo-400 hover:bg-indigo-500 py-3.5 rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1.5"
                        >
                          <Activity className="h-3.5 w-3.5" />
                          Execute Simulated Match (Sim Fill)
                        </button>
                      </div>
                    </div>

                    {/* Trades Table Display */}
                    <div className="md:col-span-7 rounded-xl bg-zinc-950 border border-zinc-900 p-4 min-h-[175px] flex flex-col justify-between">
                      <div className="space-y-2">
                        <span className="text-[9px] font-mono text-indigo-400 tracking-wider font-extrabold uppercase block">
                          📝 SIMULATED TRADING ACCOUNT PORTFOLIO HISTORY
                        </span>

                        <div className="overflow-x-auto">
                          <table className="w-full text-[10px] text-left">
                            <thead>
                              <tr className="border-b border-zinc-900 text-zinc-500 uppercase font-mono tracking-wider font-black">
                                <th className="pb-1.5 font-bold">Ref ID</th>
                                <th className="pb-1.5 font-bold">Asset</th>
                                <th className="pb-1.5 font-bold">Qty</th>
                                <th className="pb-1.5 font-bold">Price</th>
                                <th className="pb-1.5 font-bold text-right">Debit</th>
                              </tr>
                            </thead>
                            <tbody>
                              {simSandboxTrades.length === 0 ? (
                                <tr>
                                  <td colSpan={5} className="py-6 text-center text-zinc-650 italic leading-relaxed font-semibold">
                                    No sandbox simulated trades executed yet. Click buy on the left to test execution routines.
                                  </td>
                                </tr>
                              ) : (
                                simSandboxTrades.map((t) => (
                                  <tr key={t.id} className="border-b border-zinc-900 last:border-0 text-zinc-300 font-mono font-semibold">
                                    <td className="py-2 text-indigo-400 font-black">{t.id}</td>
                                    <td className="py-2 font-display text-white">{t.ticker}</td>
                                    <td className="py-2">{t.qty.toLocaleString()}</td>
                                    <td className="py-2">${t.price.toLocaleString()}</td>
                                    <td className="py-2 text-right text-rose-400">-${t.total.toLocaleString()}</td>
                                  </tr>
                                ))
                              )}
                            </tbody>
                      ...
                      {simSandboxTrades.length > 0 && (
                        <div className="flex justify-end pt-2 border-t border-zinc-900/60">
                          <button
                            onClick={() => {
                              setSimSandboxBalance(100000);
                              setSimSandboxTrades([]);
                            }}
                            className="text-[8px] font-mono font-black text-rose-400 hover:text-rose-300 uppercase cursor-pointer"
                          >
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  <div className={`p-1 rounded bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-white transition-all transform duration-300 ${isOpen ? 'rotate-180 bg-zinc-805 text-[#FE8C00]' : ''}`}>
